...
print(f"capturing evS (socks scene, k={K})...",flush=True); t0=time.time()
...
np.savez('evS.npz',bins=np.array(common,dtype=np.int32),cov=cov,mean=mean,dr_m=np.float32(DR_M))
print(f"SAVED evS.npz {len(common)} bins, min/bin={acc.min_count(bins)}",flush=True)
