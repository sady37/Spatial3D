...
print(f"capturing emptyJ (k={K})...",flush=True); t0=time.time()
...
np.savez('emptyJ.npz',bins=np.array(common,dtype=np.int32),cov=cov,mean=mean,dr_m=np.float32(DR_M))
print(f"SAVED emptyJ.npz {len(common)} bins, min/bin={acc.min_count(bins)}",flush=True)
