...
acc=BinAccumulator(k=300,n_ant=N_VIRT_ANT); bins=range(87,271)
print("capturing baseH...",flush=True); t0=time.time()
while time.time()-t0<90 and acc.min_count(bins)<300:
    f=s.get_frame(timeout=1.0)
...
np.savez('baseH.npz',bins=np.array(common,dtype=np.int32),cov=cov,mean=mean,dr_m=np.float32(DR_M))
print(f"SAVED baseH.npz {len(common)} bins, min/bin={acc.min_count(bins)}",flush=True)
