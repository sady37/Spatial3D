...
class LiveSource:
    ...
    def request_cube(self, range_bin, n_frames=30, half_win=3, timeout=6.0):
        ...
        with self._lock:
            self._cube_query = q
        print(f"[cube-query] send bin={int(range_bin)} hw={int(half_win)} n_frames={int(n_frames)}", flush=True)
        try:
        ...
        entries = q["entries"]
        print(f"[cube-query] done bin={int(range_bin)} entries={len(entries)} timeout={timeout}", flush=True)
        return entries
