...
def _fetch_cube_bg(range_bin, floor_frac, n_frames=60):
    ...
    start_t = time.time()
    try:
        print(f"[cube] fetch start bin={range_bin} n_frames={n_frames} "
              f"floor_frac={floor_frac:.2f} time={time.strftime('%H:%M:%S', time.localtime(start_t))}",
              flush=True)
        if hasattr(_src, "request_cube"):
            ...
            print(f"[cube] fetch done bin={range_bin} entries={len(ents)} "
                  f"rr={rr} str={strength} micro={micro} measured={measured} "
                  f"duration={time.time()-start_t:.1f}s", flush=True)
    except Exception as exc:
        print(f"[cube] fetch failed bin={range_bin} error={exc}", flush=True)
    finally:
        _cube_busy[0] = False
...
def _scene():
    ...
    if (down and cube_delay_ok and not _cube_busy[0] and fresh
        ...
        if rb is not None:
            ...
            _last_query_t[0] = now
            _cube_episode[0] += 1
            print(f"[cube] trigger query bin={rb} episode={_cube_episode[0]} "
                  f"delay_ok={cube_delay_ok} fresh={fresh} busy=0 time={time.strftime('%H:%M:%S', time.localtime(now))}",
                  flush=True)
    ...
    extra = {
        "cloud_z_med": cloud_wz_med,
        "cloud_below_frac": cloud_below_frac,
        "lying_state": lying_state,
        "floor_fall": floor_fall,
        "geom_flat": _geom_flat,
        "prim_ffrac": prim_ffrac,
        "w_hs": w_hs,
    }
    dec = _cleaner.decide({"down": down, "h_s": w_hs}, mlp_out,
                          cube=cube_ev, geom=None, extra=extra)
    ...
    if down or dec["trigger"] or cube_ev is not None:
        cr = _cube_result
        ti_state = (f"ti_pose={fw.label if fw else 'None'} "
                    f"ti_fprob={round(fw.falling_prob, 2) if fw else 'None'} "
                    f"ti_down={int(fw.down) if fw else 'None'} "
                    f"ti_win_valid={int(fw.win_valid) if fw else 'None'} "
                    f"ti_valid={int(fw.valid) if fw else 'None'}")
        ts_str = time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime(now))
        extra_str = (f"extra=(floor_fall={int(floor_fall)},lying={int(lying_state)},"
                     f"cloud_below={round(cloud_below_frac,2)},"
                     f"cloud_z_med={(None if cloud_wz_med is None else round(cloud_wz_med,2))})")
        _line = (f"{ts_str}.{int((now % 1) * 1000):03d} "
                 f"[fall] {fall_state:9s} down={int(down)}(w={int(w_down)}/{w_src},"
